<template>
  <div id="ForumQuestion">
    <div class="question">
      ...
      <!-- 问题描述 -->
      <div class="description">{{ questioner.forumContent }}</div>
      <div class="button">
        <!-- 添加回答按钮 -->
        <el-button
          type="primary"
    ...
    methods: {
      ...
      getInfo (id) {
        ...
        this.postRequest(questionerUrl)
          .then((res) => {
            ...
            this.questioner.forumContent = data.forumContent;
            this.questioner.answerCount = data.answerCount;
          })
          .catch((error) => {
            console.log(error);
          });
        // 更新回答内容
        let commentsUrl = `/getForumAnswerByForumContentId?forumContentId=${id}`;
      ...
      answerSubmit () {
        ...
        if (this.answerContent == '') {
          ...
          this.postRequest(answerUrl)
            .then((res) => {
              alert('提交成功');
              console.log('提交', res);
              // 关闭提交回答框
              this.answerVisi = false;
              // 刷新回答列表
              this.commentsList = res.data;
